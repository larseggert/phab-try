...
// ignored. The host is hardcoded in the literal (not via new RegExp()) so
// semgrep doesn't flag it as a dynamic-regex ReDoS risk. If PHAB_HOST
// changes, update the literal below to match. Reused across calls —
// matchAll() creates a fresh iterator each time and doesn't mutate
// lastIndex on the source regex, so a single shared instance is safe.
const PHAB_D_RE =
  /Differential Revision:\s*https?:\/\/phabricator\.services\.mozilla\.com\/D(\d+)\b/gi;

function extractDNums(text) {
  return [...new Set([...(text ?? "").matchAll(PHAB_D_RE)].map((m) => m[1]))];
}

// "Bug NNN - subject" is the canonical commit subject prefix; line-anchored
// so mid-line prose like "regression of Bug 12345" doesn't match. Shared
// between extractBugNums and bugRegex — matchAll() iterator-safe, see above.
const BUG_LINE_RE = /^Bug\s+(\d+)\b/gim;
...
function extractBugNums(text) {
  return [...new Set([...(text ?? "").matchAll(BUG_LINE_RE)].map((m) => m[1]))];
}

// Returns an object with .test() so callers use it like a RegExp.
// Static literal + string equality avoids new RegExp() with a dynamic
// argument (flagged as a ReDoS risk by static analysis tools).
function bugRegex(b) {
  const id = String(parseInt(b, 10));
  return {
    test: (text) => [...(text ?? "").matchAll(BUG_LINE_RE)].some((m) => m[1] === id),
  };
}
...
function stripPhabSuffix(t) {
  return (
    (t ?? "").replace(/\s*[-–—·•]\s*(Differential\s*[-–—·•]\s*)?Phabricator\s*$/i, "").trim() ||
    null
  );
}
...
const treeherderJobsUrl = (repo, rev) => `${TREEHERDER_BASE}/jobs?repo=${repo}&revision=${rev}`;

const treeherderPushByRevUrl = (repo, rev) =>
  `${TREEHERDER_BASE}/api/project/${repo}/push/?revision=${rev}`;

const treeherderHealthUrl = (repo, rev) =>
  `${TREEHERDER_BASE}/api/project/${repo}/push/health_summary/?revision=${rev}`;

const treeherderRecentUrl = (repo, count) =>
  `${TREEHERDER_BASE}/api/project/${repo}/push/?count=${count}`;
...
const treeherderAuthorHistoryUrl = (repo, params) =>
  `${TREEHERDER_BASE}/api/project/${repo}/push/?${params}`;

const hgRevUrl = (rev) => `${HG_TRY_BASE}/json-rev/${rev}`;
// Batch endpoint — accepts multiple hashes joined by | (URI-encoded as %7C).
// Returns { entries: [{ node, desc, parents, … }] }.
const hgShortLogBatchUrl = (hashes) =>
  `${HG_TRY_BASE}/json-shortlog/?rev=${encodeURIComponent(hashes.join("|"))}`;
