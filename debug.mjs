...
if (!rawD) { console.error("Usage: node debug.mjs <D-number> [email]"); process.exit(1); }
...
for (const p of pushes)
  console.log(`  ${p.revision.slice(0, 12)}  ${new Date(p.push_timestamp * 1000).toISOString()}  ${p.author}`);
